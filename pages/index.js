...
export default function Home() {
  ...
  /*
   * Load username and study data
   */
  useEffect(() => {
    const savedUser =
      localStorage.getItem('studyhub_username');

    if (savedUser) {
      const cleanUsername = savedUser
        .replace(/^@/, '')
        .trim()
        .toLowerCase();
      ...
      // Register/find user in MongoDB
      registerUser(cleanUsername);
    }

    /*
     * If a timer was already running,
     * restore it after refreshing the page.
     */
  ...
  const loadStudyStats = async (user) => {
    try {
      const response = await fetch(
        `/api/study?username=${encodeURIComponent(user)}`
      );

      const data = await response.json();

      if (data.success) {
        setTotalSecondsStudied(
          data.totalSeconds || 0
        );
      } else {
        console.error(
  ...
  const stopAndSaveTime = async () => {
    ...
    if (!username) {
      alert(
        'Please login before saving study time.'
      );
      return;
    }
    ...
    try {
      ...
      alert(
        '❌ Could not save your study session. Please try again.'
      );
                ...
                <div className="rounded-[29px] bg-white p-8 text-center sm:p-12">
                  ...
                  {!isActive ? (
                    ...
                    <button
                      ...
                      className="w-full rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-500 py-4 text-lg font-black text-white shadow-lg shadow-emerald-100 transition hover:-translate-y-0.5 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-60"
                    >
                      {savingSession
                        ? 'Saving...'
                        : '■ Stop & Save'}
