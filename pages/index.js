...
export default function Home() {
  ...
  // Define our new Navigation Tabs with Icons!
  const navTabs = [
    { name: 'Timer', icon: '⏱️' },
    { name: 'Vault', icon: '📁' },
    { name: 'Doubts', icon: '💬' },
  ...
  const rawLeaderboardData = [{ name: myUsername, time: totalSecondsStudied, isMe: true }];
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-start p-4 pt-8 overflow-y-auto pb-28">
          ...
          <div className="w-full max-w-md">
            ...
            <div className="flex flex-col gap-3">
              {sortedLeaderboard.map((student, index) => {
                ...
                return (
                  <div key={student.name} className="flex items-center justify-between p-4 rounded-xl bg-indigo-600 border border-indigo-400 shadow-[0_0_15px_rgba(79,70,229,0.3)]">
        ...
        {activeTab === 'Profile' && (
          <ProfileTab username={myUsername} totalSecondsStudied={totalSecondsStudied} />
        )}
      ...
      <nav className="bg-slate-800/90 backdrop-blur-md p-3 flex justify-around items-center rounded-t-3xl fixed bottom-0 w-full max-w-md left-1/2 -translate-x-1/2 border-t border-slate-700 shadow-[0_-5px_25px_rgba(0,0,0,0.5)] z-20 pb-4">
        {navTabs.map((tab) => (
          <button
            key={tab.name}
            onClick={() => setActiveTab(tab.name)}
            className={`font-medium text-[10px] sm:text-xs px-3 py-2 rounded-xl transition-all flex flex-col items-center gap-1 ${
              activeTab === tab.name 
